...
class VAS_AutoBalance_PlayerScript : public PlayerScript
{
	public:
		...
		{
			ChatHandler(Player->GetSession()).PSendSysMessage("На этом сервере запущен модуль VAS Auto Balance.");
		}
};
					...
					if (!playerList.isEmpty())
					{
						for (Map::PlayerList::const_iterator playerIteration = playerList.begin(); playerIteration != playerList.end(); ++playerIteration)
						{
							if (Player* playerHandle = playerIteration->GetSource())
							{
								...
								chatHandle.PSendSysMessage("|cffFF0000 [Автобаланс]|r|cffFF8000 %s вошел в подземелье %s. Количество игроков в автоматическом режиме: %u (Смещение сложности = %u) |r", player->GetName().c_str(), map->GetMapName(), map->GetPlayersCountExceptGMs() + PlayerCountDifficultyOffset, PlayerCountDifficultyOffset);
							}
						}
					}
				}
